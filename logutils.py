...
def logger(msg,
           request_logging_levels=None,
           filter_logging_levels=None,
           severity=None):
    ...
    try:
        if not request_logging_levels:
            return
        if (
            "debug" in request_logging_levels or (
                filter_logging_levels and (
                    "debug" in filter_logging_levels or (
                        any([x in filter_logging_levels for x in request_logging_levels]))))
        ):
